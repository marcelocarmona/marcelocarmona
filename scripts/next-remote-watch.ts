...
function colorizeLogMessage(msg: string, color?: string): string {
  switch (color) {
    case 'black':
...
const app = next({
  dev: true,
  turbopack: process.env.NEXT_FORCE_WEBPACK !== 'true',
...
app.prepare().then(() => {
  ...
  if (program.args.length > 0) {
    chokidar
      ...
      .on(program.event, async (filePathContext: string, eventContext = defaultWatchEvent) => {
  ...
  reloadRoute.all('/', (req: any, res: any) => {
  ...
  expressApp.all('*', (req: any, res: any) => handle(req, res, parse(req.url, true)))
