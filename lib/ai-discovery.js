...
function comparePostsByPublishedDate(a, b) {
  return new Date(b.date || 0).getTime() - new Date(a.date || 0).getTime()
}

function toAbsoluteMaybe(pathOrUrl) {
...
function buildPostEntry(posts, post) {
...
export async function generateLlmsFullTxt() {
  const index = await generateAiIndex()
  const postsByLocale = index.posts.reduce((acc, post) => {
