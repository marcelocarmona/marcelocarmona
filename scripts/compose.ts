...
const getAuthors = () => {
  ...
  const authorList = fs.readdirSync(authorPath).map((filename: string) => path.parse(filename).name)
  return authorList
}
...
const genFrontMatter = (answers: any) => {
  let d = new Date()
  const date = [
    d.getFullYear(),
  ...
  tagArray.forEach((tag: string, index: number) => (tagArray[index] = tag.trim()))
...
inquirer
  ...
  .then((answers: any) => {
    ...
    fs.writeFile(filePath, frontMatter, { flag: 'wx' }, (err: Error | null) => {
      if (err) {
        throw err
      } else {
