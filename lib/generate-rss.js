...
function getSortedPosts(posts) {
  return [...posts].sort((a, b) => {
    const bTime = getPostUpdatedDate(b)?.getTime() || 0
    const aTime = getPostUpdatedDate(a)?.getTime() || 0
    return bTime - aTime
  })
}
...
const generateRssItem = (post) => {
  ...
  const rssDate = formatRssDate(post.lastmod || post.date)
...
const generateRss = (posts, page = 'feed.xml', options = {}) => {
  ...
  const lastBuildDate =
    getPostUpdatedDate(sortedPosts[0])?.toUTCString() || new Date().toUTCString()
